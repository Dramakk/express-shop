...
const composeVariations = function (colors, productId, productName, postgres, callback) {
    ...
    postgres.query(`SELECT id, height, width, depth, dimensions, color FROM item 
                    where id <> ${productId} AND itemname = '${productName}' AND color IN (${colorClause}) AND isdeleted = false`,
        function (err, result) {
            if (err) {
                throw (err);
            }
...
const getVariations = function (productId, productName, postgres, callback) {
    postgres.query(`SELECT color FROM item where id <> ${productId} AND itemname = '${productName}' AND isdeleted = false GROUP BY color`,
        function (err, result) {
            if (err) {
                throw (err);
            }
...
const searchForProducts = function (searchString, postgres, callback) {
    ...
    postgres.query(`SELECT DISTINCT ON (itemname) picture, itemname, id, price FROM item 
        WHERE itemname LIKE '%${itemNameSearch}%' OR description LIKE '%${safeSearchString}%' AND isdeleted = false`, (error, result) => {
        if (error) {
            callback(error, -1);
        }
        else {
...
const getProductsByCategory = function (category, postgres, callback) {
    if (category === -1) {
        postgres.query(`SELECT DISTINCT ON (itemname) * FROM item WHERE isdeleted = false`, (error, result) => {
            if (error) {
                throw error;
            }
            else {
        ...
        postgres.query(`SELECT DISTINCT ON (itemname) * FROM item WHERE dimensions=${dim} WHERE isdeleted = false`, (error, result) => {
            if (error) {
                throw error;
            }
            else {
...
module.exports = { composeDimension, getProductData, searchForProducts, getProductsByCategory };